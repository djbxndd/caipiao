...
print("更新后的数组d的值是：\n",d)
print("更新后的数组d的维度值、秩分别是：\n",d.shape,d.ndim)
print(d.shape[0],d.shape[1])   ##引用元组的元素也是使用[]
print(type(d))
...
np.savetxt(filename+".csv",d,fmt='%d',delimiter=",")  ##第一种写入文件的方法；

d.tofile(filename+".dat",sep=",",format="%d")       ##第二种写入文件的方法；tofile()有一定的局限性，文件不分行存储；待研究
